public class Convert {
    public static String[] massRoman = new String[]{"0", "I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X",
    ...
    public static boolean isRoman (String inputLine){
        for (int i = 0; i < massRoman.length; i++) {
            if (inputLine.equals(massRoman[i])) {
                return true;
            }
        } return false;
    }
    public static int convertToArabian(String inputLine){
        for (int i = 0; i < massRoman.length; i++) {
            if (inputLine.equals(massRoman[i])) return i;
        } return 0;
    }
    public static String convertArabianToRoman(int result){

        return massRoman[result];
    }
}
